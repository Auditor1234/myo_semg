...
from baseline import TCN, ECNN, CNN2DEncoder, ECNN_Model
...
def main(model, subjects, loss_func=cross_entropy):
    ...
    acc, region_acc, split_acc = train(model, epochs, x_train, y_train, x_val, y_val, x_test, y_test, loss_func=loss_func)
    return acc
